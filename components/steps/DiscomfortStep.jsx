...
export default function DiscomfortStep({ data, setVal }) {
  ...
  return (
    <> {/* React fragment */}
      ...
      {/* outer wrapper already has 1.5 rem gap → OK */}
      <section className="space-y-6">
          ...
          <>
            ...
            <div className="mt-4">
              <p className="text-sm font-medium mb-2">
                Areas of Pain / Weakness{' '}
                <span className="text-xs font-normal">(check all that apply)</span>
              </p>
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                ...
                {dc.areas.includes('Other') && (
                  <input
                    ...
                    value={dc.otherArea}
                    onChange={(e) =>
                      setVal(['discomfort', 'otherArea'], e.target.value)
                    }
                  />
                )}
              </div>
            ...
            <div className="mt-4">
              <label className="block text-sm mb-1">
                Do you also feel numbness or tingling?
              </label>
              <select
                className="border rounded w-full"
                value={dc.numb ?? 'no'}
        ...
        <div className="mt-4">
          <label className="block text-sm mb-1">
            Have you seen anyone for this already?
          </label>
          <select
            className="border rounded w-full"
            value={dc.seen}
          ...
          {dc.seen === 'yes' && (
            <input
              ...
              value={dc.provider}
              onChange={(e) =>
                setVal(['discomfort', 'provider'], e.target.value)
              }
            />
          )}
        </div>
        ...
        <div className="mt-4">
          <label className="block text-sm mb-1">
            Do you have a sense of what set it off?
          </label>
          <select
            className="border rounded w-full"
            value={dc.trigger}
        ...
        <div className="mt-4">
          <label className="block text-sm mb-1">
            Anything that reliably eases or aggravates it?{' '}
