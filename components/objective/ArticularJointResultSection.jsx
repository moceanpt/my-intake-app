  ...
  const articularJointResult = React.useMemo(() => {
    try {
      ...
      const hasRequiredMetrics = requiredMetrics.every(metric => {
        ...
        return metricData && (metricData.left !== null || metricData.right !== null);
      });
                ...
                <div style={{ background: 'white', padding: '1rem', borderRadius: '8px', border: '1px solid #e5e7eb' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.75rem' }}>
                    {group.metrics.map((metric, metricIndex) => {
                      ...
                      let hasPain = false;
                      // Check for pain indicators in the nested data structure
                      const metricData = data[metricKey];
                      if (metricData) {
                        if (isLeft && metricData.leftPain) hasPain = true;
                        else if (isRight && metricData.rightPain) hasPain = true;
                        else if (!isLeft && !isRight && metricData.leftPain) hasPain = true;
                      }
